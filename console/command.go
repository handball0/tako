...
var RootCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if err := executeCommands(cmd, args); err != nil {
			Echo.Fatalw("❌ 错误: 命令执行失败", "error", err)
		}
	},
}
...
func executeCommand(cmdInfo *CommandInfo, rootCmd *cobra.Command, args []string) error {
	...
	if err != nil {
		...
		cmdInfo.Error = err
		Echo.Error("命令执行失败", "command", cmdInfo.Command.Use, "error", err, "duration", cmdInfo.EndTime.Sub(cmdInfo.StartTime))
	} else {
		cmdInfo.Status = CommandStatusCompleted
		Echo.Info("命令执行成功", "command", cmdInfo.Command.Use, "duration", cmdInfo.EndTime.Sub(cmdInfo.StartTime))
	}
	close(errChan)
	return nil
}
